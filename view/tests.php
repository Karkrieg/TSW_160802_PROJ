...
<div class="row">
    <div class="col-lg-12">
        <h1 class="text-center mt-2">Testy</h1>
        <div id="inside">
...
<!-- Dodawanie testu realizowane za pomocą test-creator.php -->
...
<!-- Część odpowiedzialna za modyfikację testu (brak na tę chwilę możliwości edycji) -->
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="PUT" id="form_update">
                <div class="modal-header">
                    <h2 class="modal-title text-center">Edycja testu</h2>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="form-floating mb-3 mt-3">
                        <select class="form-select" name="gid" id="opt-group">
                            <!-- Grupy -->
                        </select>
                        <label for="opt-group">Grupa</label>
                    </div>
                    <div class="form-floating mb-3 mt-3">
                        <input type="text" name="tytul" class="form-control" id="etitle" placeholder="Tytuł" />
                        <label for="egrupa">Tytuł</label>
                    </div>
                </div>
                <div class="modal-footer border">
                    <input type="button" class="btn mx-auto btn-danger w-50" data-bs-dismiss="modal" value="Anuluj" />
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        function show() {
            let i = 0;
            $.ajax({
                url: "http://localhost/TSW_160802_PROJ/api/test/read_all.php",
                method: "POST",
                dataType: "JSON",
                ...
                success: function(response) {
                    $('#inside').html("");
                    let testArray = response.data;
                    let content =
                        '<div class="col-lg-12 mt-3">' +
                        '<a href="test-creator.php" class="btn btn-success btn-xs w-100 create" id="dodaj" >&plus; Dodaj test &plus;</a>' +
                        '</div>';
                    if (response.data != undefined) {

                        content += '<table class="table text-center table-bordered table-striped table-dark mt-3">' +
                            '<thead><tr>' +
                        ...
                        testArray.forEach(function(row, index, array) {
        ...
        $(document).on('click', '.delete', function() {
            ...
            tstid = JSON.stringify(tstid);
            $.ajax({
                url: "http://localhost/TSW_160802_PROJ/api/test/delete.php",
                method: "DELETE",
                dataType: "JSON",
                contentType: "application/json",
                data: tstid,
        ...
        $(document).on('submit', '#form_update', function(event) {
            ...
            console.log(myJSON);
            $.ajax({
                url: "http://localhost/TSW_160802_PROJ/api/test/update.php",
                method: "PUT",
                dataType: "json",
                contentType: "application/json",
                data: myJSON,
